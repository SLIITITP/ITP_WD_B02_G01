...
import { CREATE_TASK, DELETE_TASK, FAIL_REQUEST, GET_TASK_LIST, GET_TASK_OBJ, MAKE_REQUEST, UPDATE_TASK } from "./ActionType"
...
export const RemoveTask = (code) => {
    return (dispatch) => {
        dispatch(makeRequest);
        axios.delete('/api/task/' + code).then(res => {
            dispatch(deleteTask());
        }).catch(err => {
            dispatch(failRequest(err.message));
        })
...
export const TaskUpdate = (code, data) => {
    return (dispatch) => {
        dispatch(makeRequest);
        axios.put('/api/task/' + code, data).then(res => {
            dispatch(updateTask());
...
export const FetchTaskObj = (code) => {
    return (dispatch) => {
        dispatch(makeRequest);
        axios.get('/api/task/' + code).then(res => {
            const tasklist = res.data;
